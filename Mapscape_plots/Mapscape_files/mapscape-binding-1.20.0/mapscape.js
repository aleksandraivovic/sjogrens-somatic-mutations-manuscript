HTMLWidgets.widget({
    ...
    initialize: function(el, width, height) {
        ...
        var defaults = {
            ...
            legendWidth: 350, // tb14 edited, larger legend size allows for easier navigation of tree
            legendTitleHeight: 20, // tb14 edited, larger legend size allows for easier navigation of tree
            ...
            legendSpacing: 8, // spacing between legend items, tb14 edited-less spacing allows for larger legend items
    ...
    renderValue: function(el, x, instance) {
        ...
        // tb14 edit, get static tree image
        dim.static_tree_image = "data:image/png;base64," + curVizObj.userConfig.static_tree_image;
        ...
        dim.innerRadius = dim.viewDiameter/4.5; // radius for centre circle (where anatomy will go), tb14 edit-increased anat image size
        dim.circBorderWidth = 3; // width for circular border width
        
        // - 8, - 20, - 3 for extra space
        dim.oncoMixWidth = ((dim.outerRadius - dim.circBorderWidth - dim.innerRadius)/2) - 45; // tb14 edited-decrease size of clone visualisation, allows for more samples without crowding
        dim.treeWidth = ((dim.outerRadius - dim.circBorderWidth - dim.innerRadius)/2) - 60; // tb14 edited-decrease size of trees, allows for more samples without crowding
        dim.radiusToOncoMix = dim.innerRadius + dim.oncoMixWidth/2 + 65; // radius to oncoMix centre, tb14 edited-increased radius of circle of clone visualisation, allows for more samples without crowding
        dim.radiusToTree = dim.innerRadius + dim.oncoMixWidth + dim.treeWidth/2 + 65; // radius to tree centre, tb14 edited-increased radius of circle of trees, allows for more samples without crowding

        // legend layout
        dim.legendHeight = dim.viewDiameter;
        dim.legendTreeWidth = dim.legendWidth + 1; // width of the tree in the legend, tb14 edited - increased tree size for easier navigation 
        ...
        $.when(curVizObj.imgDimDeferred.promise()).then(function() {
            ...
            var tree_height = curVizObj.data.tree_height - 2; // tb14 edited, how large trees on plot are, shrunk from default
            ...
            // PLOT Static Tree IN LEGEND tb14 edit adding display of second static image for additional visualizaitons

            // anatomy title
            var static_treeTitle_top = tree_top + dim.legendTreeWidth + dim.legendSpacing;
            var static_treeTitle = curVizObj.userConfig.static_tree_title.split(" ");
            static_treeTitle.forEach(function(word, word_i) {
                viewSVG.append("text")
                    .attr("class", "legendTitle")
                    .attr("x", dim.legendWidth/2) 
                    .attr("y", static_treeTitle_top + word_i*(dim.legendTitleHeight + dim.legendTitleSpacing))
                    .attr("dy", "+0.71em")
                    .attr("text-anchor", "middle")
                    .attr("transform", "translate(" + dim.viewDiameter + ",0)")
                    .attr("color", dim.legendTitleColour)
            ...
            // static tree image
            // tb14 edit, if/else statement added to fix spacing issues when anatomy image is taller than it is wise
            if (dim.legend_image_height > dim.legend_image_width) {
                dim.static_tree_height = dim.legend_image_width*0.6;
            } else {
                dim.static_tree_height = dim.legend_image_width;
            }
            var static_treeImage_top = static_treeTitle_top + 
                                   static_treeTitle.length*(dim.legendTitleHeight);
            viewSVG.append("image")
                .attr("xlink:href", dim.static_tree_image)
                .attr("x", dim.viewDiameter + dim.legend_image_top_l.x)
                .attr("y", static_treeImage_top)
                .attr("width", dim.legend_image_width)
                .attr("height", dim.static_tree_height)

            // PLOT ANATOMY IN LEGEND, tb14 edit-shifted below static tree

            // anatomy title
            var anatomyTitle_top = static_treeImage_top + dim.static_tree_height + dim.legendSpacing;
            ...
            // PLOT MIXTURE CLASSIFICATION, tb14 edit, commenting out as does not seem useful
            ...
            // PLOT CIRCLE BORDER, tb14 edited-commented out to declutter plot
